...
frappe.ui.form.on('Job Sheet', {
	onload_post_render: function(frm) {
		$(frm.fields_dict['html_image'].wrapper)
		...
		<img  id="sourceImage"   src="/files/small.png" style="max-width: 900px; max-height: 80%;"  crossorigin="anonymous" /> \
		<img  id="sampleImage"   src="/files/small.png"  style="max-width: 900px; max-height: 100%; position: absolute;" crossorigin="anonymous" /> \
		</div>');
...
frappe.ui.form.on('Job Sheet', {
    setup:function(frm){
        frm.check_activity_duplicate = function(frm,row){  
            console.log(row)
        frm.doc.Operations.forEach(item=>{
           if(!row.activity_name==''){
            if((row.activity_name==item.activity_name) && (!row.idx==item.idx)){
row.activity_name=''; 
frm.refresh_field('operations')
                frappe.throw(__(`${row.activity_name} opertaion already exist row ${item.idx}`))
            }
        }})
        }}
...
// frm.check_activity_duplicate (frm ,row)
// console.log(row)

// frappe.ui.form.on("Job Sheet", "refresh", function(frm) {
...
frappe.ui.form.on("Job Sheet", {
    refresh: function(frm, cdt, cdn) {
        ...
        if (!frm.is_new() && frm.doc.docstatus === 1) {
            frm.add_custom_button(__("Create Work_Order"), function() {
                frappe.new_doc('Work_Order', {
                    "customer_name": cur_frm.doc.customer_name,
                    "job_order_no": cur_frm.doc.name,
                    "company": cur_frm.doc.company,
                    "mobile_no": cur_frm.doc.mobile_no,
                    "posting_date": cur_frm.doc.posting_date,
                    "landline_no": cur_frm.doc.landline_no,
                    ...
                    "engine_no_chassis_no__drive_unit": cur_frm.doc.engine_no_chassis_no__drive_unit,
                    "color": cur_frm.doc.color,
                    "color_name": cur_frm.doc.color_name,
                    "brand": cur_frm.doc.brand,
                    "year": cur_frm.doc.year,
                    "model": cur_frm.doc.model,
                    "meter_reading": cur_frm.doc.meter_reading,
                    "fuel_level": cur_frm.doc.fuel_level,
                    "remarks": cur_frm.doc.remarks, 
                    "warranty_start_date": cur_frm.doc.warranty_start_date,
                    "service_contract_start_date": cur_frm.doc.service_contract_start_date,
                    "warranty_end_date": cur_frm.doc.warranty_end_date,
                    "service_contract_end_date": cur_frm.doc.service_contract_end_date,
                });
            });
        }}
    });
...
// frappe.ui.form.on("Job Sheet", "color" {
// 	refresh(frm) {

// var color_hex = frm.doc.color

// var n_match  = ntc.name("color_hex");
// var n_name       = n_match[1];
// frm.set.value(cdt, cdn, 'color_name', n_name);

//         refresh_field("Job Sheet");
...
frappe.ui.form.on('Job Sheet', {
    onload(frm) {
      frm.set_query('items', 'operations', () => {
            return {
                filters: {
                    item_group: ["in" , ['Body Shop - Service' , 'Car Care  - Service' , 'Electrical  - Service' , 'Mechanical  - Service',]]
                }
            }
        })
    }
...
// let markerArea = new markerjs2.MarkerArea(target);
